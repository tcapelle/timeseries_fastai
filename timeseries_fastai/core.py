...
__all__ = ['maybe_unsqueeze', 'show_array', 'TSeries', 'get_ucr', 'load_df_ucr', 'get_x', 'get_y']
...
def maybe_unsqueeze(x):
    ...
    if type(x) is np.ndarray: return x[None,:] if len(x.shape)==1 else x
    if type(x) is Tensor: return x.unsqueeze(0) if len(x.shape)==1 else x
    else: return None
